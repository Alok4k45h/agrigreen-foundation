...
const getThemeStyles = (theme: string) => {
  switch (theme) {
    ...
    default: return { text: "text-foreground", bg: "bg-secondary", border: "border-border", activeBorder: "border-foreground", shadow: "shadow-none" };
  }
};
...
export default function FaqSection() {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-background transition-colors duration-300" id="faq">
          ...
          >
                ...
                {FAQS.map((faq, index) => {
                  ...
                  return (
                    <motion.div
                      key={index}
                      initial={false}
                      animate={{ backgroundColor: isOpen ? "var(--secondary)" : "transparent" }}
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${isOpen ? styles.bg + " " + styles.text : "bg-secondary text-muted-foreground group-hover:text-foreground"}`}>
                            <faq.icon size={20} />
                          </div>
                          <span className={`text-lg font-bold transition-colors ${isOpen ? "text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}>
                            {faq.question}
                          </span>
                        </div>
                        
                        <div className={`p-2 rounded-full transition-all duration-300 ${isOpen ? "rotate-180 bg-background text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}>
                          <ChevronDown size={18} />
                        </div>
                      </button>
            ...
            <div className="mt-8 text-center">
              <p className="text-muted-foreground">
                ...
                <Link href="/contact" className="text-agri font-semibold hover:underline decoration-agri/50">
